...
export class PaymentTaskService implements IPaymentTaskService {
  ...
  async updateFailedTBRPayments(): Promise<void> {
    const now = new Date()
    this.logger.info(
      'Starting TBR payment status update job for failed payments',
      {
    ...
    const failedTransactions = await this.tbrTransactionModel.findAll({
      where: {
    ...
    if (failedTransactions.length === 0) {
      this.logger.info('No failed TBR payments found, skipping job', {
    ...
    this.logger.info(
      `Found ${failedTransactions.length} failed TBR payments to retry`,
      {
        context: LOGGING_CONTEXT,
      },
    )
    ...
    const adverts = await this.advertModel.findAll({
      where: {
        transactionId: {
          [Op.in]: failedTransactions.map((t) => t.id),
        },
      },
    })
    // Now we need to call postTbrPayment again for each failed transaction
    for (const advert of adverts) {
      try {
        ...
        this.logger.info('TBR payment retried successfully', {
          advertId: advert.id,
          transactionId: advert.transactionId,
          context: LOGGING_CONTEXT,
        })

        await this.tbrTransactionModel.update(
          { status: TBRTransactionStatus.CREATED },
          { where: { id: advert.transactionId as string } },
        )
      } catch (error) {
        this.logger.error('TBR payment retry failed', {
          advertId: advert.id,
          transactionId: advert.transactionId,
          error: error,
          context: LOGGING_CONTEXT,
