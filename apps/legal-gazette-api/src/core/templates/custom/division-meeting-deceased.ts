...
export function getDivisionMeetingDeceasedTemplate(model: AdvertModel): string {
  const isValid = validateDivisionMeetingModel(model)

  if (!isValid) {
    throw new InternalServerErrorException(
      'Invalid model provided for division meeting deceased template',
    )
  }
  ...
  const dateCell = getTableCell(formatDate(meetingDate, 'dd. MMMM yyyy'))

  const nationalIdCell = getTableCell(`kt. ${nationalId}`)
  const timeCell = getTableCell(`${formatDate(meetingDate, 'HH:mm')}`)
