...
export function getRecallDeceasedTemplate(model: AdvertModel): string {
  ...
  const outro = getElement(
    `Hér með er skorað á alla þá, sem telja til skulda eða annarra réttinda á hendur framangreindu dánarbúi eða telja til eigna í umráðum þess, að lýsa kröfum sínum fyrir undirrituðum skiptastjóra í búinu innan tveggja mánaða frá fyrri birtingu þessarar innköllunar. ${getElement(`Kröfulýsingar skulu sendar skiptastjóra ${settlement?.liquidatorRecallStatementType !== ApplicationRequirementStatementEnum.LIQUIDATORLOCATION ? 'með rafrænum hætti á netfangið ' : 'að '} ${settlement?.liquidatorRecallStatementLocation || ''}`)}`,
  )
