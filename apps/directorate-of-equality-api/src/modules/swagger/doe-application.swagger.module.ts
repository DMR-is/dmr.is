 ...
 * `Application` tag, so they generate into a single client class. Everything
 * listed here is behind that boundary; the unauthenticated statistics surface
 * has its own aggregate (`DoeStatisticsSwaggerModule`).
 *
 * Aggregating them here (the mirror of `DoeWebSwaggerModule`) is what keeps
 * runtime registration and the published document from drifting apart:
 * `AppModule` and `SWAGGER_CONFIG` both name this module, so a new
 * applicant-facing api module cannot be routed without also being documented.
 * `swagger-coverage.spec.ts` enforces that.
 *
