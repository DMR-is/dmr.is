 ...
 * because the fixture it reads is app-owned: `db/seeders/data/` is regenerated by
 * `scripts/refresh-wage-gap-fixtures.ts` and consumed by three seeders. A library
 * spec reaching into an app's `db/` would invert the dependency direction the
 * extraction exists to establish.
 *
