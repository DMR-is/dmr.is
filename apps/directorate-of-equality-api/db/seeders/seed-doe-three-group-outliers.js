...
// Load source data ----------------------------------------------------------
// ⚠️ The DEMO sheet, not the raw capture. `excel-import-test-res.json` is a
// pristine parse of the source workbook, and as captured it is COMPLIANT under
// the rule that replaced the ±band — óskýrt 2,11% í óhag karla, empty
// lágmarksmengi — so it demonstrated nothing. `rich-demo-sheet.json` is that
// sheet with every woman's pay scaled by 0,9, generated by
// `nx run directorate-of-equality-api:refresh-wage-gap-fixtures` together with
// its chart and its decomposition, so the employee rows, the chart and the
// frozen snapshot all describe the same cohort. Result: óskýrt 7,84% í óhag
// kvenna, lágmarksmengi of 6.
const excel = require('./data/rich-demo-sheet.json')
// The chart the API would return for these rows, built by the real
// `buildChartFromEmployeePoints`. Shaped like the old `analysis` object so the
// existing call sites keep working.
const analysis = { regularHourlyWageByScoreAll: excel.chart }
...
function resultSql() {
  ...
  // The frozen decomposition, computed by the real engine from the same demo
  // sheet this seeder inserts — see scripts/refresh-wage-gap-fixtures.ts.
  // óskýrt 7,84% í óhag kvenna, over the 3,9% benchmark, lágmarksmengi of 6.
  // `outliersSql()` below derives its rows from this very object, so the three
  // groups and the figure above them can never disagree.
  const wageGapSnapshot = wageGapFixtures.richSheet
...
VALUES (${escStr(resultId)}, ${escStr(SAL_REPORT_ID)}, 3.90, 'v4',
...
function outliersSql() {
  ...
  const outlierValues = filtered.map((o, i) => {
    const empId = empIdByOrdinal.get(o.ordinal)
    if (!empId)
      throw new Error(`Lágmarksmengi references unknown ordinal ${o.ordinal}`)
