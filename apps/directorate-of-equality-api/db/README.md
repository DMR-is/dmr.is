...
- **Partner API:** there is no required preview, so nobody can opt in honestly. The submission passes `postponeUnexplainedOutliers`: if outliers are detected and no `outlierGroups` were sent, the report is postponed rather than refused, and the response's `status` and `unexplainedOutlierOrdinals` say so. A clean payroll files as `SUBMITTED` either way.
...
- **Submit-side outlier detection.** `report-create.service.ts.createSalary()` runs the same `detectOutliers(parsed, threshold)` helper as the preview, then `resolveOutlierPostponement()` decides the outcome — one of three cases:

  - `outliersPostponed` set (island.is, after previewing): at least one outlier must be detected — postponing nothing is refused — and the report lands in `POSTPONED`; `outlierGroups` is ignored.
  ...
  Threshold is re-read from `config` at submission time, so on island.is a small drift between preview and submit is possible — rejection in that case just means "re-run preview". On the partner API there is no required preview: `POST /api/v1/partner/reports/salary-analysis` is an optional dry run with its own rate-limit bucket, and the submission's response carries `status` and `unexplainedOutlierOrdinals`, taken from the set detection just produced. Postponement applies to the whole report; the applicant resolves it later via the outliers edit endpoint on either channel.
