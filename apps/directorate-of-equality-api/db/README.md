...
- **`POSTPONED`** — applies to `SALARY` reports only. The company submitted with all outliers deferred (`outliers[]` rows persisted with null explanation columns) — deliberately on island.is or by an admin filing on the company's behalf (`outliersPostponed = true`), or automatically on the partner API when outliers were detected and no groups were sent (see "Postponement deadline"). The report is **not pickable** by reviewers — `assign()` rejects this status, `approve()` rejects this status. The applicant resolves the postponement via `PUT /api/v1/application/reports/:providerId/outliers` (or the partner API's `PUT /api/v1/partner/reports/:providerId/outliers`, which delegates to the same `editOutliers`), which fills in the explanation fields and transitions the row to `SUBMITTED` (emitting a `STATUS_CHANGED` + an `EDITED` event). Reviewers can read the report and its content while it sits here, but cannot act on it.
...
- **`WITHDRAWN`** — a report retired before any reviewer acted on it. **Terminal state.** Reached two ways, both before the report became live work:
  - **Auto-withdrawn on sibling resubmission.** When a company submits a new report of a given `type`, `report-create.service.ts` silently withdraws any still-`SUBMITTED` predecessor of the same type for that company (`withdrawInflightSibling()` in `report-finalize.service.ts`): the old row flips to `WITHDRAWN` and a `WITHDRAWN` `report_event` is emitted on it with `related_report_id` pointing at the new replacing report (mirroring `SUPERSEDED`). A prior report in `IN_REVIEW` is **not** auto-withdrawn — a reviewer is mid-workflow — so the new submission is rejected with 409 instead. `POSTPONED` depends on the channel: on island.is it also 409s, since deferring there was the applicant's deliberate choice; the partner API passes `withdrawPostponedSibling`, so a `POSTPONED` sibling **filed through the partner API** is withdrawn and replaced like a `SUBMITTED` one (a vendor correcting a payroll error should not have to explain figures it knows are wrong first). A `POSTPONED` report filed on island.is still 409s a partner filing.
...
Only applies to a report that lands in `POSTPONED` status (see "Report lifecycle"). How it gets there depends on the channel:

- **island.is:** the submitter **opts in** at submit time with `outliersPostponed = true`, having seen the outliers in the preview.
- **Partner API:** there is no required preview, so nobody can opt in honestly. The submission passes `postponeUnexplainedOutliers`: if outliers are detected and no `outlierGroups` were sent, the report is postponed rather than refused, and the response's `status` and `unexplainedOutlierOrdinals` say so. On a replay (same `providerId`), `status` is the earlier report's current status and `unexplainedOutlierOrdinals` is absent — that call ran no detection; recover the set with `GET /api/v1/partner/reports/:providerId/outliers`. A clean payroll files as `SUBMITTED` either way.

Once postponed:

- The submitter then has **3 months** from submission to provide the explanations (via `PUT /api/v1/application/reports/:providerId/outliers`, or `PUT /api/v1/partner/reports/:providerId/outliers` on the partner API — both resolve `POSTPONED → SUBMITTED`).
...
- **Outlier-preview endpoint.** Lands as `POST /api/v1/application/reports/salary-analysis` in the `application` module. Takes the unsaved parsed payload, computes employee scores with `report/lib/employee-scores.ts`, and runs `analyzeSalaryPayload()` (`report-statistics/lib/salary-analysis.ts`): one `computeWageGapDecomposition` against the configured benchmark percent, whose lágmarksmengi (`selectMinimumSet`) is the set of employees the company must account for — two-directional, and empty for an already-compliant company. Returns that set plus the gender-vs-score chart so the company can verify before submitting.
- **Submit-side outlier detection.** `report-create.service.ts.createSalary()` recomputes the same lágmarksmengi as the preview (`computeDetectedOutlierOrdinals()` — the same decomposition and benchmark), then `resolveOutlierPostponement()` decides the outcome — one of three cases:

  - `outliersPostponed` set (island.is or an admin filing, after previewing): at least one outlier must be detected — postponing nothing is refused — and the report lands in `POSTPONED`; `outlierGroups` is ignored.
  - outliers detected, no groups, and the channel passed `postponeUnexplainedOutliers` (the partner API): postponed rather than refused. Never an error — that is the point for a caller that sends the payroll once.
  - otherwise: the union of every group's `employeeOrdinals` must cover the detected set exactly — no extras, none missing, no ordinal in two groups.

  The benchmark is re-read from `config` at submission time, so on island.is a small drift between preview and submit is possible — rejection in that case just means "re-run preview". On the partner API there is no required preview: `POST /api/v1/partner/reports/salary-analysis` is an optional dry run with its own rate-limit bucket, and the submission's response carries `status` and `unexplainedOutlierOrdinals`, taken from the set detection just produced. Postponement applies to the whole report; it is resolved later via the outliers edit endpoint **of the channel it was filed on** — each channel's `PUT …/outliers` only finds its own reports, so a `POSTPONED` report from island.is can only be finished on island.is. A report an admin files as `POSTPONED` (`providerType = SYSTEM`) has no such endpoint today: nothing can move it to `SUBMITTED`, and a reviewer can only deny it.
