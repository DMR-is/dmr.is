 ...
 */
/**
 * Exported so the scoring model's validator can run the *real* capacity rules
 * rather than restate them. It re-stated them once and drifted: `MAX_CRITERIA`
 * is 5, four mandatory job-based types plus one personal already sit at the
 * cap, and a model that split one criterion in two reported VALID and then
 * threw here on both preview and submit.
 */
  ...
  for (const employee of parsed.employees) {
    ...
    if (computeRegularWages(employee) <= 0) {
      issues.add(
        ...
        `${employeeLabel(
          employee.ordinal,
        )}: engin regluleg laun; grunnlaun og viðbótarlaun mega ekki vera 0 samanlagt`,
        { ordinal: employee.ordinal },
      )
    }
