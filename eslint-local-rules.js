...
module.exports = {
  ...
  /**
   * This rule enforces that services using @Cacheable or @CacheEvictTopics decorators
   * have the required cache manager injection and imports:
   *
   * Required imports:
   ...
   *
   * This ensures that the cache manager is properly initialized and available
   * for the decorators to use.
   */
  'require-cache-manager-injection': {
    meta: {
      type: 'problem',
      docs: {
        description:
          'require cache manager injection when using caching decorators',
        category: 'Possible Errors',
        recommended: true,
      },
      messages: {
        missingCacheImports:
          "Class uses @Cacheable or @CacheEvictTopics but is missing required imports. Add: import { Cache, CACHE_MANAGER } from '@nestjs/cache-manager'",
        missingCacheManagerInjection:
          'Class uses @Cacheable or @CacheEvictTopics but is missing cache manager injection in constructor. Add: @Inject(CACHE_MANAGER) private readonly cacheManager: Cache',
      },
      schema: [],
    },
    create: function (context) {
      ...
      let hasCacheManagerImport = false
      ...
      function hasCachingDecorator(node) {
        // Check class decorators
        if (
          hasDecorator(node.decorators, 'Cacheable') ||
          hasDecorator(node.decorators, 'CacheEvictTopics')
        ) {
          return true
        }
        ...
        return methods.some(
          (method) =>
            hasDecorator(method.decorators, 'Cacheable') ||
            hasDecorator(method.decorators, 'CacheEvictTopics'),
        )
      }

      function hasInjectCacheManager(node) {
        const classBody = node.body?.body || []
        ...
        return params.some((param) => {
          // For parameter properties (with modifiers like private, readonly),
          // decorators are on the param itself, not param.parameter
          const decorators = param.decorators || []

          // Looking for parameter with @Inject(CACHE_MANAGER) decorator
          const hasInject = decorators.some((decorator) => {
            const expr = decorator.expression
            if (
              expr?.type === 'CallExpression' &&
              ...
              return args.some(
                (arg) =>
                  arg.type === 'Identifier' && arg.name === 'CACHE_MANAGER',
              )
            }
            return false
          })

          // For parameter properties, the type annotation is directly on the param
          // For regular parameters, it's also on param.typeAnnotation
          let hasType = false
          ...
          // Check if param has a parameter property (TSParameterProperty)
          // In that case, the type is on param.parameter.typeAnnotation
      ...
      return {
        // Track imports
        ImportDeclaration: (node) => {
          if (node.source.value === '@nestjs/cache-manager') {
            const specifiers = node.specifiers || []
            specifiers.forEach((spec) => {
              if (spec.type === 'ImportSpecifier') {
                if (spec.imported.name === 'Cache') {
                  hasCacheImport = true
                }
                if (spec.imported.name === 'CACHE_MANAGER') {
                  hasCacheManagerImport = true
                }
              }
            })
          }
        ...
        // Check class-level usage
        ClassDeclaration: (node) => {
          const decorators = node.decorators
          if (!decorators) return

          // Check if class uses caching decorators
          if (!hasCachingDecorator(node)) return

          // Check for required imports
          if (!hasCacheImport || !hasCacheManagerImport) {
            context.report({
              node,
              messageId: 'missingCacheImports',
            })
          }

          // Check for cache manager injection in constructor
          if (!hasInjectCacheManager(node)) {
            context.report({
              node,
              messageId: 'missingCacheManagerInjection',
            })
          }
        },
      }
